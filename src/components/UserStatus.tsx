...
import { FarcasterState } from '../store/Farcaster';

type UserStatusProps = {
    // setAuthState: React.Dispatch<React.SetStateAction<AuthState | undefined>>,
} & typeof Localization.actionCreators & ApplicationState;
...
const UserStatus : React.FunctionComponent<UserStatusProps> = props => {
    ...
    return <>
        ...
        <div onClick={() => {/*setIsMenuExpanded(true)*/}} className="user-status">
            <div className="user-status-icon" style={{ backgroundImage: `url(${props.farcaster.user!.pfp!.url})` }}/>
            <div className="user-status-text">
                <div className="user-status-username">{props.farcaster.user!.username}</div>
                <div className="user-status-info">{props.farcaster.user!.displayName}</div>
            </div>
        </div>
    </>;
}
