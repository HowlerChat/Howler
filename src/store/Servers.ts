...
export interface ServersState {
    authToken: string;
    servers: { [serverId: string]: ServerInfoState };
...
function* handleConnectionRequest(request: RequestConnectionAction) {
    ...
    let response: { connection: WebSocket, error: any } = { connection: new WebSocket(""), error: undefined };
...
export const unloadedState: ServersState = { servers: {}, isConnecting: false, connection: undefined, authToken: '', isLoading: false, lastRefreshedServerId: null };

export const serversReducer: Reducer<ServersState> = (state: ServersState | undefined, incomingAction: Action): ServersState => {
    if (state === undefined) {
