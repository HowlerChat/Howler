...
function* handleSpaceRequest(request: RequestSpaceAction) {
    ...
    let space: Space = yield call(getSpace(request.spaceId)(request.token));
...
export const unloadedState: SpacesState = { spaces: [], isLoading: false };

export const spacesReducer: Reducer<SpacesState> = (state: SpacesState | undefined, incomingAction: Action): SpacesState => {
    if (state === undefined) {
