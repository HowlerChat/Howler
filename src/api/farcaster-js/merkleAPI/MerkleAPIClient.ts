...
export class MerkleAPIClient {
   ...
   */
  constructor(
    walletOrAuthToken: ethers.Wallet | { secret: string; expiresAt?: number },
    {
      logger = silentLogger,
      axiosInstance,
    ...
    if (walletOrAuthToken instanceof ethers.Wallet) {
  ...
  public async *fetchCastsInThread(
    ...
    while (true) {
      ...
      // prep for next page
      if (!response.data.next || response.data.next.cursor === undefined) {
        break;
      }
  ...
  private async _authHeader(params: V2AuthBody | V2AuthBody1): Promise<string> {
    ...
    const signature = Buffer.from(
      ethers.utils.arrayify(await this.wallet.signMessage(payload))
    ).toString("base64");
